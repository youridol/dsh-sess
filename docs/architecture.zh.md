...
原生会话行省略号菜单（ui-workspace）渲染的是封闭组件列表（重命名 / 分叉 /
归档），对第三方没有 slot，因此 dsh-sess 在 DOM 层扩展它——只为自己的入口：

- 捕获阶段监听会话行省略号按钮的打开；
- MutationObserver 在 portal 化的 `role="menu"` 出现后扩展它——仅当菜单确实含
  **归档**行时，克隆该行并在其后直接插入 **删除会话** 条目（相同哈希类名、
  危险色垃圾桶图标、本地化文案）；
- 选中该条目会关闭原生菜单并打开插件自有的确认弹窗（`row-delete.tsx`），
  走与设置页完全相同的宿主删除路径；
- 会话 id 取自该行 React fiber 的 `memoizedProps.node.id`（并按宿主字符集
  校验）；无法证实 id 时跳过该入口——破坏性操作绝不信任可见文本。

这是对其它包组件树的 UI 层扩展，刻意保持防御式并记录在案：仅结构匹配（归档文案
双语匹配）、每次菜单打开幂等、带注入标记、任何未知形态下静默失效（未来
ui-workspace 若重构行结构，该入口可能消失，需随之适配）。破坏性操作本身仍走官方
宿主服务。
